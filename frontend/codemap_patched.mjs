#!/usr/bin/env node
// Patched Code Map: safer entry detection + correct traverse usage
// Usage: node codemap_patched.mjs [project-root]
// This script scans a JavaScript/TypeScript project, analyzes its files, and generates a code map with various insights.
// It outputs a manifest of files, a graph of dependencies, and a summary in Markdown format.
...
const looksLikeComponentName = (name) => !!name && /^[A-Z]/.test(name);
const stripQueryAndHash = (spec) => spec.split('?')[0].split('#')[0];
...
async function parseFile(file){
  ...
  const info = {
    file: normalizeSlashes(path.relative(projectRoot, file)),
  ...
  info.imports = await Promise.all(rawImports.map(async (src) => {
    ...
    return { source: src, resolvedFile: resolved ? normalizeSlashes(path.relative(projectRoot, resolved)) : null };
  }));
...
function isEntryCandidate(f){
  const nf = normalizeSlashes(f);
  if (!nf.includes('/src/')) return false;
  return ['.js','.jsx','.ts','.tsx'].some(ext => nf.endsWith(ext));
}
...
function buildGraph(manifest){
  ...
  const indexEntries = files.filter(f => {
    const bn = path.basename(f, path.extname(f));
    return INDEX_BASENAMES.includes(bn) && f.includes('/src/');
  });
  const chosen = indexEntries.length ? indexEntries : files.filter(isEntryCandidate).slice(0,1);
