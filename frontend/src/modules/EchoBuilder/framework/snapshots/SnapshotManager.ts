...
export class SnapshotManager<T = unknown> {
  ...
  save(name: string, payload: T): Snapshot<T> {
    const s: Snapshot<T> = { id: uid(), name, at: Date.now(), payload };
    const next = [s, ...this.list()].slice(0, 200);
    saveJSON(KEY, next);
    return s;
  }
  load(id: string) { return this.list().find(s => s.id === id); }
  diff(a: T, b: T) {
    try { return JSON.stringify(b, null, 2).split("\n").filter(line => !JSON.stringify(a, null, 2).includes(line)); }
    catch { return []; }
  }
}
