...
const write=(f,c)=>{ensureDir(path.dirname(f));fs.writeFileSync(f,c,"utf8");log("+",rel(f));};
const writeIfMissing=(f,c)=>fs.existsSync(f)?log("=",rel(f),"(exists)"):write(f,c);
const patch=(f,fn)=>{ if(!fs.existsSync(f))return false; const a=fs.readFileSync(f,"utf8"); const b=fn(a);
  if(a!==b){fs.writeFileSync(f,b,"utf8");log("~",rel(f));return true;} log("=",rel(f),"(no changes)"); return false; };

ensureDir(libDir); ensureDir(widgetsDir);

/* 1) HUD bus + hook */
writeIfMissing(path.join(libDir,"hudBus.js"), `import { useEffect } from "react";
...
/* 2) global CSS tokens (includes text sizes) */
writeIfMissing(path.join(libDir,"textVars.css"), `:root{
  --panel-radius:16px;
  ...
  --accent:#16E0FF;
  --tab-height:36px; --tab-radius:10px;
  --tab-active-bg:rgba(255,255,255,0.10); --tab-border:1px solid rgba(255,255,255,0.12);
  --mode:dark;
...
/* 3) ensure widget templates in registry */
...
/* 4) create compact widgets if missing */
...
const writeIfMissingCompact=(f,c)=>fs.existsSync(f)?log("=",rel(f),"(exists)"):write(f,c);
